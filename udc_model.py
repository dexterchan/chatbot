  ...
  def model_fn(features, targets, mode):
    ...
    if mode == tf.contrib.learn.ModeKeys.EVAL:
      ...
      probs, loss = model_impl(
          hparams,
          mode,
          tf.concat(axis=0, values=all_contexts),
          tf.concat(axis=0, values=all_context_lens),
          tf.concat(axis=0, values=all_utterances),
          tf.concat(axis=0, values=all_utterance_lens),
          tf.concat(axis=0, values=all_targets))

      split_probs = tf.split(axis=0, num_or_size_splits=10, value=probs)
      shaped_probs = tf.concat(axis=1, values=split_probs)
