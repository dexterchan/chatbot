...
def dual_encoder_model(
  ...
  with tf.variable_scope("rnn") as vs:
    ...
    rnn_outputs, rnn_states = tf.nn.dynamic_rnn(
        cell,
        tf.concat(axis=0, values=[context_embedded, utterance_embedded]),
        sequence_length=tf.concat(axis=0, values=[context_len, utterance_len]),
        dtype=tf.float32)
    encoding_context, encoding_utterance = tf.split(axis=0, num_or_size_splits=2, value=rnn_states.h)
